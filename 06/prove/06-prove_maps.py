...
def is_anagram(word1, word2):
    ...
    # Handle cases where the words are not the same length.
    if len(word1) != len(word2):
        return False

    # Create a dict from the first word.
    # The character will be the key and count will be the value.
    anagram_dict = {}
    for char in word1:
    ...
    # If the character in word 2 is in the anagram_dict
    # Then decrement the count by 1.
    for char in word2:
        if char in anagram_dict:
            anagram_dict[char] -= 1
    
    # If any of the counts are not equal to 0 then return false.
