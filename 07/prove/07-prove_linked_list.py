...
class LinkedList:
    ...
    def insert_tail(self, value):
        ...
        # If its empty, then point both head and tail to node.
        if self.head is None:
            self.head = new_node
            self.tail = new_node
        # If the list is not empty, then only self.tail will be affected
        else:
            new_node.prev = self.tail   # Connect new node to previous tail
            self.tail.next = new_node   # Connect previous tail to new node
    ...
    def remove_tail(self):
        ...
        """
        # Set head and tail to None if its an empty list.
        if self.head == self.tail:
            self.head = None
            self.tail = None
        
        # If the list has more than one item in it, then only the tail will be affectd.
    ...
    def remove(self, value):
        ...
        """
        # Search for node that matches value.
        curr = self.head
        while curr is not None:
            if curr.data == value:
                # If at the end of the list then call remove tail
                if curr == self.tail:
                    self.remove_tail()
                    return
                # if its at the beginning of the list then call remove head
                elif curr == self.head:
                    self.remove_head()
                    return
    ...
    def __reversed__(self):
        ...
        curr = self.tail
        while curr is not None:
            yield curr.data  # Replace this when you implement your solution
            curr = curr.prev
